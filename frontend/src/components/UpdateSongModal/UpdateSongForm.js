...
function UpdateSongForm({ song, setShowModal }) {
  ...
  const [url, setUrl] = useState(song.url || song.url.name || '');
  const [imageUrl, setImageUrl] = useState(song.imageUrl|| song.imageUrl.name || '');
  ...
  useEffect(() => {
    ...
    if (!/^\S+.(wav|mp3|aac|flac)$/.test(url.name || url))
  ...
  return (
    ...
    {isLoading ? (
        ...
        <form onSubmit={handleSubmit}>
          <h1>
            Update this <br /> little hatchling!
          </h1>
          <input
            type="text"
            value={title}
          ...
          <label for="file-input-audio">
            {url.name ? url.name : url || "Choose an audio..."}
          </label>
